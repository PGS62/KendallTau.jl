#This file is a copy of StatsBase test/pairwise.jl with the minimal changes required to work
...
#        @test_throws Union{ArgumentError,MethodError} pairwise(f, [Int[]], [Int[]])#CHANGED
#        @test_throws Union{ArgumentError,MethodError} pairwise!(f, zeros(1, 1), [Int[]], [Int[]])#CHANGED
    ...
    @testset "missing values handling interface" begin
        ...
        if VERSION >= v"1.5" # Fails with UndefVarError on Julia 1.0
         #   @test_throws Union{ArgumentError,MethodError} pairwise(f, xm, ym, skipmissing=:pairwise)#CHANGED
         #   @test_throws Union{ArgumentError,MethodError} pairwise(f, xm, ym, skipmissing=:listwise)#CHANGED

            res = zeros(Union{Float64, Missing}, length(xm), length(ym))
#            @test_throws Union{ArgumentError,MethodError} pairwise!(f, res, xm, ym, skipmissing=:pairwise)#CHANGED
#            @test_throws Union{ArgumentError,MethodError} pairwise!(f, res, xm, ym, skipmissing=:listwise)'CHANGED
        end
    ...
    @testset "cor corner cases" begin
        ...
        for sm in (:pairwise, :listwise)
            ...
            @test res ≅ [1.0 NaN
                         NaN 1.0]
            if VERSION >= v"1.5"
#                @test_throws ArgumentError pairwise(cor, [[missing, missing, missing],#CHANGED
#                                                          [missing, missing, missing]],
#                                                    skipmissing=sm)
            end
        end
